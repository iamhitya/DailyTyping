...
const INITIAL_WORDS_COUNT = 500
const BUFFER_WORDS_COUNT = 100
const DEFAULT_APPEND_WORDS_COUNT = 200
  ...
  switch (action.type) {
    ...
    case 'back': {
      if (action.alt) {
        ...
        if (state.progress.charIndex === 0) {
          if (state.progress.wordIndex === 0) return
          const prevWord = state.words[state.progress.wordIndex - 1]
          if (!prevWord) return
          state.progress.charIndex = prevWord.length - 1
          state.progress.wordIndex--
        } else {
    ...
    case 'keydown': {
      ...
      const targetKey = currentWord[state.progress.charIndex]
...
function handleKeyDown(
  ...
  if (state.progress.charIndex < currentWord.length - 1) {
    state.progress.charIndex++
  } else {
  ...
  if (state.progress.wordIndex > state.words.length - BUFFER_WORDS_COUNT) {
    appendWords(state)
  }
}
...
function getRandomQuotes(data: QuoteData[], charCount: number) {
  ...
  while (words.length < wordCount && data.length > 0) {
...
function appendWords(state: State, count = DEFAULT_APPEND_WORDS_COUNT) {
  const newWords =
    state.dataName === 'Quotes'
      ? getRandomQuotes(state.data as unknown as QuoteData[], count)
      : getRandomWords(state.data, count)
...
function reset(state: State) {
  ...
  // set new words
  state.words = []
  appendWords(state, INITIAL_WORDS_COUNT)
}
...
export function getInitialState(): State {
  ...
  const soundEnabled = getLocalStorage('soundEnabled', 'true', validateBoolean) !== 'false'
