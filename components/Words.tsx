...
export function Words({ words, progress, errorLocations }: WordsProps) {
  ...
  useEffect(() => {
    if (!shouldScrollRef.current || !wordsRef.current) return
...
const Word = memo(
  function Word({ word, errorsInWord, activeCharIndex, isTyped, isCurrent }: WordProps) {
    return (
      <div
  ...
  (prevProps, nextProps) => {
    ...
    return (
      ...
      prevProps.errorsInWord === nextProps.errorsInWord
    )
  }
)
